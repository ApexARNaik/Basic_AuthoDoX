...
const showAdminLoginButtonApp = document.getElementById('showAdminLoginButtonApp'); // Added for button in App Header
...
promptFileInput.addEventListener('change', () => {
    promptFileNameDisplay.textContent = promptFileInput.files[0] ? promptFileInput.files[0].name : 'No file chosen';
});
responseFileInput.addEventListener('change', () => {
    responseFileNameDisplay.textContent = responseFileInput.files[0] ? responseFileInput.files[0].name : 'No file chosen';
});
...
function showSection(sectionToShow) {
    [connectSection, appSection, adminLoginSection, adminSection].forEach(s => { if(s) s.style.display = 'none'; });
    if (sectionToShow) sectionToShow.style.display = 'block';
}
function showConnectSection() {
    showSection(connectSection);
    connectStatus.textContent = ''; adminLoginStatus.textContent = ''; adminPasswordInput.value = '';
}
function showAdminLogin() {
    showSection(adminLoginSection); adminLoginStatus.textContent = '';
}

// --- 6. IPFS UPLOAD ---
async function uploadToIPFS(fileBlob, fileName = "content.dat") {
    if (!PINATA_JWT) throw new Error("IPFS config missing (VITE_PINATA_JWT Secret)."); // Updated error msg
    ...
    try {
        ...
        if (!res.ok) throw new Error(`Pinata Error: ${resData.error?.reason || res.statusText}`);
        console.log("IPFS Upload:", resData);
        return resData.IpfsHash;
    } catch (error) { console.error('IPFS Upload Error:', error); throw error; }
}
...
async function packageAndUploadInput(textInput, fileInput, baseFileName) {
    ...
    if (text && !file) {
        ...
        const fileCid = await uploadToIPFS(file, `${baseFileName}_${file.name}`);
...
async function connectWallet() {
    if (typeof window.ethereum === 'undefined') { connectStatus.textContent = 'Please install MetaMask!'; return; }
    connectStatus.textContent = 'Connecting... Please approve in MetaMask.'; connectButton.disabled = true;
    try {
        ...
        try { ownerAddress = await contract.owner(); console.log("Owner:", ownerAddress); }
        catch (ownerError) { console.error("Fetch Owner Error:", ownerError); connectStatus.textContent = 'Error fetching owner.'; connectButton.disabled = false; return; }
        ...
        loadReportsButton.disabled = false; registerStatusDisplay.textContent = '';
        connectButton.disabled = false;
        await loadReports();
    } catch (error) {
        console.error("Connection Error:", error);
        connectStatus.textContent = 'Connection failed. User rejected or error occurred.'; connectButton.disabled = false; showConnectSection();
    }
}
function disconnectWallet() {
    provider = null; signer = null; contract = null; currentUserAddress = null; ownerAddress = null;
    walletAddressDisplay.textContent = 'Not Connected'; console.log("Disconnected"); showConnectSection();
}

async function loadReports() {
    if (!contract) { console.error("Contract not init."); reportList.innerHTML = '<li>Error: Connect wallet first.</li>'; return; }
    reportList.innerHTML = '<li><p>🔄 Loading proofs from the blockchain...</p></li>'; loadReportsButton.disabled = true;
    try {
        ...
        if (numReports === 0) { reportList.innerHTML = '<li><p>📬 No proofs registered yet.</p></li>'; loadReportsButton.disabled = false; return; }
        reportList.innerHTML = '';
        for (let i = numReports - 1; i >= 0; i--) { // Newest first
            ...
            loadAndDisplayPreview(i, report.contentCid, listItem.querySelector('.content-preview'));
        }
    } catch (error) { console.error("Load Reports Error:", error); reportList.innerHTML = '<li>❌ Error loading proofs. Check console.</li>';
    } finally { loadReportsButton.disabled = false; }
}

async function handleRegisterProof() {
    if (!contract || !signer) { alert("Connect wallet first."); return; }
    registerStatusDisplay.textContent = '🚀 Processing inputs & uploading to IPFS...'; registerProofButton.disabled = true;
    let promptCid = "", contentCid = "", metadataJson = {}, metadataCid = "";
    const optionalChatLink = chatLinkInput.value.trim();
    const authorAddress = await signer.getAddress();
    try {
        console.log("Uploading prompt..."); promptCid = await packageAndUploadInput(promptTextInput, promptFileInput, "prompt");
        console.log("Uploading content..."); contentCid = await packageAndUploadInput(responseTextInput, responseFileInput, "content");
        metadataJson = {
            name: `Autho.D.oX Proof #${Date.now()}`, description: "Immutable proof of AI content generation.",
            image: (responseFileInput.files[0] && /\.(jpg|jpeg|png|gif|svg|webp)$/i.test(responseFileInput.files[0].name)) ? `ipfs://${contentCid}` : null,
            attributes: [
                { trait_type: "Prompt CID", value: promptCid }, { trait_type: "Content CID", value: contentCid },
                { trait_type: "Author", value: authorAddress }, { trait_type: "Timestamp", value: Math.floor(Date.now() / 1000) },
                { trait_type: "Chat Link", value: optionalChatLink || "N/A" } ] };
        if (metadataJson.image === null) delete metadataJson.image;
        console.log("Uploading metadata JSON..."); const metadataBlob = new Blob([JSON.stringify(metadataJson)], { type: 'application/json' });
        metadataCid = await uploadToIPFS(metadataBlob, "metadata.json");
        registerStatusDisplay.textContent = '🚀 Sending transaction... Please confirm in MetaMask.';
        console.log("Calling registerProof:", promptCid, contentCid, metadataCid, optionalChatLink);
        // Corrected call based on final Registry contract
        const tx = await contract.registerProof(promptCid, contentCid, metadataCid, optionalChatLink);
        registerStatusDisplay.textContent = `⏳ Transaction sent! Waiting... Hash: ${tx.hash.substring(0, 10)}...`; console.log("Tx Sent:", tx.hash);
        await tx.wait(1); registerStatusDisplay.textContent = '✅ Proof registered successfully!'; console.log("Tx Confirmed!");
        promptTextInput.value = ''; promptFileInput.value = null; responseTextInput.value = ''; responseFileInput.value = null; chatLinkInput.value = '';
        ...
        await loadReports();
    } catch (error) { console.error("Register Proof Error:", error); let displayError = parseContractError(error, `Registration Error: ${error.message || 'Unknown'}`); registerStatusDisplay.textContent = displayError;
    } finally { registerProofButton.disabled = false; }
}

async function loadAndDisplayPreview(id, cid, previewElement) {
    if (!cid) { previewElement.innerHTML = '<small>No content CID.</small>'; return; }
    ...
    try {
        ...
        if (contentType?.startsWith('image/')) {
            ...
            const jsonData = await response.json(); let html = '<small><b>Preview:</b><br>';
            if (jsonData.text) html += `<i>Text:</i> ${jsonData.text.substring(0, 100)}${jsonData.text.length > 100 ? '...' : ''}<br>`;
            if (jsonData.fileCid) html += `<i>File:</i> <a href="${IPFS_GATEWAY}${jsonData.fileCid}" target="_blank">${jsonData.fileName || 'Linked File'}</a>`;
        ...
        } else { previewElement.innerHTML = `<small><i>Content:</i> <a href="${url}" target="_blank">View File</a></small>`; }
    } catch (error) { console.error(`Preview Error (CID ${cid}):`, error); previewElement.innerHTML = `<small style="color:red;">Error loading preview.</small>`; }
}
...
function handleAdminLogin() {
    ...
    if (pass === ADMIN_PASSWORD) {
        ...
        } else { adminLoginStatus.textContent = "Error: Connected wallet is not owner."; }
    } else { adminLoginStatus.textContent = "Incorrect code."; }
...
// --- UTILITY ---
function parseContractError(error, defaultMessage, specific1 = null, specific2 = null) {
    console.log("Raw error:", error); let msg = defaultMessage;
    if (error.reason) msg = error.reason;
    else if (error.error?.message) msg = error.error.message;
    else if (error.data?.message) msg = error.data.message;
    else if (error.message) msg = error.message;
    msg = msg.replace("execution reverted: ","").replace("VM Exception while processing transaction: reverted with reason string ","").replace("Internal JSON-RPC error.","").replace(/error=.*{(.*)}.*}, method=.*$/, "$1").replace(/\"message\":\"(.*?)\".*$/, "$1");
    msg = msg.replace(/^Error\([^)]*\)\s*/, '').replace(/^Panic\([^)]*\)\s*/, '').trim();
    if (specific1 && msg.toLowerCase().includes(specific1.toLowerCase())) return `Error: ${specific1}`;
    if (specific2 && msg.toLowerCase().includes(specific2.toLowerCase())) return `Error: ${specific2}`;
    if (error.code === 4001 || msg.includes("User denied")) return "Transaction rejected.";
    if (error.code === -32000 && msg.includes("insufficient funds")) return "Error: Insufficient funds for gas.";
